...
from logic.loads.water_heater import (  # noqa: E402
    MODE_AUTO,
    MODE_BOOST,
    MODE_HC_ONLY,
    MODE_OFF,
...
class TestRougeHp:
    ...
    def test_rouge_hp_never_engages_however_good_the_buffer(self, base_inputs, thr):
        """ROUTEUR: this used to be allowed and is now refused outright.

        The original weighed a worst-case blended cost: 30 kWh of solar
        already banked meant two more hours at 0.706 €/kWh still averaged
        0.036 €/kWh, so it engaged. The rule is now that no automatic
        decision spends Rouge peak at all — closing the contactor commits
        two hours that a single cloud turns into grid draw, with no way to
        stop.
        """
        d = decide(
            replace(
    ...
    def test_rouge_hp_does_not_break_a_cycle_already_running(self, base_inputs, thr):
        """The hardware hold outranks the lock — stopping is impossible."""
        started = base_inputs.now - timedelta(minutes=30)
        d = decide(
            replace(
                base_inputs,
                tempo_color="Rouge", is_hc=False,
                signal_currently_on=True, signal_on_at=started,
            ),
            thr,
        )
        assert d.signal_switch_on is True

    def test_a_manual_boost_still_overrides_rouge(self, base_inputs, thr):
        """The user spending their own money knowingly is not our business."""
        d = decide(
            replace(base_inputs, mode=MODE_BOOST, tempo_color="Rouge", is_hc=False),
            thr,
        )
        assert d.signal_switch_on is True
...
class TestHcWindow:
    ...
    def test_01h00_tank_below_floor_now_defers_to_a_sunny_tomorrow(
        self, base_inputs, thr
    ):
        """ROUTEUR: the 48 °C floor alone no longer justifies a night cycle.

        It used to fire on the floor and never consult the forecast, which
        is what heated the tank on four nights followed by days producing
        16-22 kWh. A 15 kWh day leaves ~3 kWh of surplus, well past the
        2 kWh needed here.
        """
        d = decide(
            self._at(base_inputs, 1, 0,
    ...
    def test_01h00_tank_below_floor_fires_when_tomorrow_is_poor(
        self, base_inputs, thr
    ):
        """Measured: a day under 12 kWh exports almost nothing."""
        d = decide(
            self._at(base_inputs, 1, 0,
                     tank_middle_c=46.0, energy_needed_kwh=2.0,
                     forecast_tomorrow_kwh=8.6),
            thr,
        )
        assert d.signal_switch_on is True
        assert "main" in d.reason

    def test_01h00_comfort_floor_fires_whatever_the_weather(self, base_inputs, thr):
        """Nobody takes a cold shower to save fifteen cents."""
        d = decide(
            self._at(base_inputs, 1, 0,
                     tank_middle_c=41.0, energy_needed_kwh=2.0,
                     forecast_tomorrow_kwh=25.0),
            thr,
        )
        assert d.signal_switch_on is True
        assert "confort" in d.reason

    def test_01h00_fills_the_tank_when_tomorrow_is_rouge(self, base_inputs, thr):
        """Tomorrow's sun will be there but unusable: we refuse Rouge peak."""
        d = decide(
            self._at(base_inputs, 1, 0,
                     tank_middle_c=46.0, energy_needed_kwh=2.0,
                     forecast_tomorrow_kwh=25.0, tempo_next_color="Rouge"),
            thr,
        )
        assert d.signal_switch_on is True
        assert "Rouge" in d.reason
    ...
    def test_03h30_still_below_floor_fires_when_the_sun_cannot_be_relied_on(
        self, base_inputs, thr
    ):
        # Forecast may have revised cloudier since 01:00, or the 01:00 cycle
        # undershot — last chance to fit a 2 h cycle before HC ends.
        # ROUTEUR: same two floors as 01:00, hence the poor forecast here.
        d = decide(
            self._at(base_inputs, 3, 30,
                     tank_middle_c=46.0, energy_needed_kwh=1.5,
                     forecast_tomorrow_kwh=9.0),
            thr,
        )
        assert d.signal_switch_on is True
    ...
    def test_checkpoint_tolerance_plus_minus_5_min(self, base_inputs, thr):
        ...
        # ROUTEUR: poor forecast, so the checkpoint has something to do.
        d = decide(
