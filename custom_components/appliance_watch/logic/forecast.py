...
# ROUTEUR: what the house swallows before anything reaches a controllable
# load. Measured over 45 days on this installation: days producing under
# 12 kWh exported a median of 0.9 kWh — nothing usable — while days at or
# above 16 kWh exported a median of 5.0 kWh. The daily *totals* cannot be
# compared directly (the house draws 36 kWh a day against 18 kWh produced,
# yet still exports at midday), so this threshold stands in for the shape of
# the day rather than its balance.
DEFAULT_SELF_CONSUMPTION_KWH: float = 12.0


def expected_surplus_kwh(
    forecast_kwh: float | None,
    self_consumption_kwh: float = DEFAULT_SELF_CONSUMPTION_KWH,
) -> float | None:
    """Roughly how much of a forecast day will actually be exportable."""
    if forecast_kwh is None:
        return None
    return max(0.0, forecast_kwh - self_consumption_kwh)
...
def solar_can_cover(
    ...
    """Will that day leave enough *surplus* to cover a heating need?

    Replaces comparing the forecast to the need directly, which called an
    8.6 kWh day "covered" for a 1.5 kWh need — when in reality such a day
    exports almost nothing, the house having absorbed it all.

    Tri-state on purpose: ``None`` means no forecast, and must not be read
    as "no" — the caller heats rather than gambles.
    """
    surplus = expected_surplus_kwh(forecast_kwh, self_consumption_kwh)
    if surplus is None:
        return None
    if energy_needed_kwh <= 0:
        return True
    return surplus >= safety_factor * energy_needed_kwh
