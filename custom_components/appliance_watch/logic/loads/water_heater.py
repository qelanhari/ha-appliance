...
try:                                              # pragma: no cover — import shim
    ...
    from ..forecast import solar_can_cover, will_likely_cover_heating_need
    ...
    from forecast import (  # type: ignore[no-redef]
        solar_can_cover,
...
class Thresholds:
    ...
    # ROUTEUR: the share of a forecast day the house swallows before any
    # surplus appears. See forecast.DEFAULT_SELF_CONSUMPTION_KWH.
    pv_self_consumption_kwh: float = 12.0
    ...
    # ROUTEUR: a second, lower floor. Above it, a night top-up is an
    # *optimisation* and must yield to a sunny tomorrow; below it, it is
    # comfort and happens whatever the weather. Measured: five nights in 150
    # days heated at off-peak rates, and four of them were followed by days
    # producing 16-22 kWh — the sun would have done it for free. Worse, a tank
    # filled to 54.5 °C at dawn cannot absorb any of that solar, which then
    # leaves as export. At ~0.4 °C/h of drift a tank at 46 °C at midnight is
    # still at ~43 °C by morning, which is plenty for a shower on 270 L.
    hc_comfort_floor_c: float = 42.0
...
def _rouge_hp_locked(i: Inputs, thr: Thresholds) -> bool:
    ...
    Closing the contactor commits the appliance for two hours. One cloud in
    those two hours and we draw at 0.706 €/kWh with no way to stop — so
    "solar only" cannot be guaranteed here, and we abstain rather than bound
    the damage. The original weighed it against a blended-cost cap; the rule
    now is simply that no automatic decision spends Rouge peak.

    A cycle already under way is not affected: the hardware hold is checked
    before this, and stopping it is impossible anyway. An explicit manual
    boost is not affected either — that is the user spending their own money
    knowingly.
    """
    return i.tempo_color == "Rouge" and not i.is_hc and not _in_signal_hold(i, thr)
...
def _hc_checkpoint_action(i: Inputs, thr: Thresholds) -> Decision | None:
    ...
    if _near(now_t, thr.hc_main_time, thr.hc_checkpoint_tolerance_min):
        # ROUTEUR: comfort first — below this floor the weather is irrelevant.
        if mid < thr.hc_comfort_floor_c:
            return Decision(
                True, False,
                f"HC 01:00 · tank_middle {mid:.1f}°C < "
                f"{thr.hc_comfort_floor_c:.0f}°C — plancher de confort",
                "heat_hc",
            )
        # ROUTEUR: above it, a top-up is an optimisation and gives way to a
        # sunny tomorrow. The original tested this floor alone, before ever
        # consulting the forecast — which is what heated the tank on four
        # nights followed by 16-22 kWh days.
        if mid < thr.hc_main_floor_c and not solar_can_wait:
            why = ("demain Rouge : le solaire sera inutilisable"
                   if tomorrow_is_rouge else
                   "prévision solaire insuffisante ou inconnue")
            return Decision(
                True, False,
                f"HC 01:00 main · tank_middle {mid:.1f}°C < "
                f"{thr.hc_main_floor_c:.0f}°C et {why}",
                "heat_hc",
            )
        if forecast_poor:
            return Decision(
    ...
    if _near(now_t, thr.hc_last_chance_time, thr.hc_checkpoint_tolerance_min):
        # ROUTEUR: same two floors as 01:00 — comfort unconditional, top-up
        # only when the sun cannot be relied on.
        if mid < thr.hc_comfort_floor_c:
            return Decision(
                True, False,
                f"HC 03:30 · tank_middle {mid:.1f}°C < "
                f"{thr.hc_comfort_floor_c:.0f}°C — plancher de confort",
                "heat_hc",
            )
        if mid < thr.hc_last_chance_floor_c and not solar_can_wait:
            return Decision(
                True, False,
                f"HC 03:30 last chance · tank_middle {mid:.1f}°C < "
                f"{thr.hc_last_chance_floor_c:.0f}°C et solaire non garanti",
                "heat_hc",
            )
        return None
...
def _decide_impl(i: Inputs, thr: Thresholds) -> Decision:
    ...
    if _rouge_hp_locked(i, thr):
        return Decision(
            False, False,
            "Rouge HP · aucune charge pilotée sur le réseau",
            "wait",
        )
    ...
    # 3. ROUTEUR: the Rouge HP branch used to live here, allowing solar heating
    # while a worst-case blended-cost cap held. It is now a lock placed above,
    # so nothing reaches this point on a Rouge peak hour.
    ...
    if _surplus_covers_heater(i, thr):
        # ROUTEUR: deliberately still the original test here, not the
        # surplus-aware one. This branch decides whether to use surplus that
        # is *already measured*; the forecast only guards against committing
        # two hours into a collapsing day. Tightening it would push heating
        # back into the night, which is the very thing being fixed. The
        # surplus model belongs to the night checkpoints, where the whole
        # decision is a bet on tomorrow.
